...
- **Multi-device Spotify usage is untested.** The engine targets whatever
  Spotify reports as the active device without explicit targeting;
  switching between devices mid-session hasn't been exercised.
- **No accessibility audit has been done.** Screen reader support,
  keyboard navigation, and contrast haven't been checked at any point.
...
Resolved since this list was last accurate: tokens are now encrypted at
rest via Electron's `safeStorage` API (`spotifyAuth.ts`), and the poll
loop now respects Spotify's `Retry-After` header on a 429 rather than
retrying blindly (`spotifyApi.ts`, `linkEngine.ts`).
